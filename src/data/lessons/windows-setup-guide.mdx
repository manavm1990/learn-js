...
- If the output shows `main`, you're all set—your system uses the modern standard
- If the output shows `master` or is blank, you need to update it
...
If VS Code will not open, check your Start menu for "Visual Studio Code" or try running the installer again as administrator. For Bun commands not found, restart your terminal and try again. If issues persist, try the optional Node.js installation as a fallback. Git recognition issues may require checking for Git Bash in your Start menu or restarting your computer. If you cannot create the Dev folder, verify you are in your user folder with appropriate permissions.
