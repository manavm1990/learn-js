...
- Render that array as a list using **render helper functions** and `.map().join("")`
...
### 1. Where We Are: Validated Forms + Render Helpers
...
- **Render helper functions** that turn objects into HTML strings
...
**Goal:** Treat each **validated submission** as an object in an array, and render a list from that array with render helper functions. No localStorage yet.
...
- Render all requests below the form using render helper functions
...
#### Step 5: Request Rendering Helpers
...
3. Use **render helper functions** for repeated UI (`RequestCard`, `RequestList`, `EmptyState`)
...
- Clear, helper-function-based rendering of the list
...
#### 2. Requirements: State & Rendering Helpers
...
- Add render helper functions (you may reuse/adjust from practice):
...
2. **Rendering Helpers:**  
  Name at least **two** render helper functions you wrote in this homework.  
...
- Combined **validated forms**, **render helper functions**, and **localStorage**
